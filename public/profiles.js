...
const Profiles = (() => {
  const KEY = 'python-course-profiles-v1';
  ...
  class Store {
    constructor(storage,count){
      ...
      if(!this.data.profiles.length){
        let legacy=null;
        for(const key of ['python-step-guest-v2','python-step-v1']){
          try{const candidate=JSON.parse(storage?.getItem(key)||'null');if(candidate&&typeof candidate==='object'){legacy=candidate;break;}}catch{}
        }
        const p={id:this.id(),name:'Learner',state:cleanState(legacy,count)};
        this.data.profiles.push(p);this.data.activeId=p.id;this.write();
      }
    ...
    reload(){
      ...
      try{
        ...
        for(const p of db.profiles){
          ...
          used.add(p.id);list.push({id:p.id,name:p.name.trim().slice(0,40),state:cleanState(p.state,this.count)});
        }
    ...
    create(name,state=fresh()){
      ...
      const p={id:this.id(),name,state:cleanState(state,this.count)};
      this.data.profiles.push(p);this.activeId=p.id;this.data.activeId=p.id;this.write();return this.state();
    }
    exportData(){return JSON.stringify({format:'python-lessons-profile',version:1,name:this.current().name,state:this.state()},null,2);}
    importData(text){
      ...
      if(!file||file.format!=='python-lessons-profile'||file.version!==1||typeof file.name!=='string'||!file.name.trim()||!file.state||typeof file.state!=='object'||Array.isArray(file.state)||!file.state.drafts||!file.state.passed)throw Error('Choose a Python lessons profile backup.');
      ...
      return this.create(name,file.state);
    }
  }
